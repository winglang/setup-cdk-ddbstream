export default interface extern {
  readStream: (event: ApiRequest, ctx: ReadStreamContext) => Promise<ApiResponse>,
}
...
export interface ITable extends IClient {
  readonly connection: () => Connection;
  readonly setStreamConsumer: (handler: (arg0: StreamRecord) => Promise<void>, options?: (StreamConsumerOptions) | undefined) => void;
}
export class Table extends Resource implements ITable {
  readonly connection: () => Connection;
  readonly setStreamConsumer: (handler: (arg0: StreamRecord) => Promise<void>) => void;
}
export interface ReadStreamContext {
  readonly streamsTable: Table;
}
